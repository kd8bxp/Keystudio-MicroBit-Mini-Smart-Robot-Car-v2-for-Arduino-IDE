...
void loop() {
  ...
  */
  for (uint16_t i=0; i<4096; i +=8) {
      ...
      pwm.setPWM(BLUE, 0, (i + (4096/16)) % 4096 ); //fade down
    ...
    for (uint16_t i=4096; i>0; i -=8) {
      ...
      pwm.setPWM(BLUE, 0, (i + (4096/16)) % 4096 ); //fade up
     // pwm.setPWM(BLUE, (i + (4096/16)) % 4096, 0 );
    delay(10);
    }
    pwm.setPWM(BLUE, 4096, 0);  //off
    for (uint16_t i=0; i<4096; i +=8) {
    
      pwm.setPWM(RED, 0, (i + (4096/16)*RED) % 4096 ); //fade down
     // pwm.setPWM(BLUE, (i + (4096/16)*BLUE) % 4096, 0 );
    delay(20);
    }
    pwm.setPWM(RED, 4096, 0);  //off
    for (uint16_t i=0; i<4096; i +=8) {
      ...
      pwm.setPWM(GREEN, 0, (i + (4096/16)*GREEN) % 4096 ); //fade down
     // pwm.setPWM(BLUE, (i + (4096/16)*BLUE) % 4096, 0 );
    delay(20);
    }
    pwm.setPWM(GREEN, 4096, 0);  //off
